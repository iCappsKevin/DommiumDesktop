...
import { applyAssertMethod } from '../../actions/Inspector';
...
export default class SelectedElement extends Component {
  ...
  render() {
    const { applyClientMethod, setFieldValue, sendKeys, assert, selectedElement, sendKeysModalVisible, showSendKeysModal,
      hideSendKeysModal, assertModalVisible, showAssertModal, hideAssertModal, selectedElementId: elementId, sourceXML, elementInteractionsNotAvailable } = this.props;
    ...
    return <div>
      ...
      <Row justify="center" type="flex" align="middle" gutter={10} className={styles.elementActions}>
        <Col>
          <ButtonGroup size="small">
            ...
            <Button disabled={!elementId} id='btnAssertElementText' onClick={() => showAssertModal()}>Assert</Button>
      ...
      <Modal title='Assert Text'
        visible={assertModalVisible}
        okText='Assert'
        cancelText='Cancel'
        onCancel={hideAssertModal}
        onOk={this.handleAssert}>
        <Input placeholder='What text should this element display?' value={assert} onChange={(e) => setFieldValue('assert', e.target.value)} />
      </Modal>
    </div>;
  }
}
