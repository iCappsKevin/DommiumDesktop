...
import {
  SET_SOURCE_AND_SCREENSHOT, QUIT_SESSION_REQUESTED, QUIT_SESSION_DONE,
...
export default function inspector(state = INITIAL_STATE, action) {
  switch (action.type) {
    ...
    case HIDE_SEND_KEYS_MODAL:
      let nextState = omit(state, 'sendKeysModalVisible');
    ...
    case HIDE_ASSERT_MODAL:
      nextState = omit(state, 'assertModalVisible');
      return omit(nextState, 'action.assert');
